...
public class VoterRegistration {
    ...
    public void reqReg(ActionEvent event) {
        ...
        String s4 = mobo.getText().trim();
        String s5=region.getText().trim();// mobile number

        // Validate name
        if (s1.length() != 10) {
            errorMessage.setText("Username must be exactly 10 characters.");
            return;
        }
        ...
        // Validate password
        if (s2.length() < 8) {
            errorMessage.setText("Password should be at least 8 characters.");
            return;
        }
        ...
        try {
            ...
            request.setFrom(s1);
            request.setTo("REGISTER_VOTER");
            request.setText(s1 + ";" + s2 + ";" + s4+ ";" + s5);
